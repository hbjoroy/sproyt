...
impl ChatRepository for SqliteChatRepository {
    ...
    fn store_media<'a>(
        ...
        Box::pin(async move {
            ...
            .bind(upload.channel_id.to_string())
            .bind(upload.actor.to_string())
            .fetch_optional(&self.pool)
            .await
            .map_err(sql_error)?;
            if allowed.is_none() {
            ...
            sqlx::query("insert into media_objects(id,owner_id,channel_id,storage_key,original_filename,content_type,size_bytes,sha256,width,height,analysis_status,analysis_metadata,created_at) values(?,?,?,?,?,?,?,?,?,?,?,?,?)")
                .bind(media.id.to_string()).bind(media.owner_id.to_string()).bind(media.channel_id.to_string()).bind(format!("db:{}", media.id)).bind(&media.original_filename).bind(&media.content_type).bind(media.size_bytes as i64).bind(&media.sha256).bind(media.width.map(i64::from)).bind(media.height.map(i64::from)).bind(&media.analysis_status).bind(media.analysis_metadata.to_string()).bind(media.created_at).execute(&mut *tx).await.map_err(sql_error)?;
            ...
            if let Some(preview) = upload.preview {
                sqlx::query("insert into media_variants(media_id,variant,content_type,size_bytes,width,height,content,created_at) values(?,'preview',?,?,?,?,?,?)")
                    .bind(media.id.to_string()).bind(preview.content_type).bind(preview.content.len() as i64).bind(i64::from(preview.width)).bind(i64::from(preview.height)).bind(preview.content).bind(media.created_at).execute(&mut *tx).await.map_err(sql_error)?;
            }
    ...
    fn load_media_preview<'a>(
        ...
        Box::pin(async move {
            let row = sqlx::query("select v.content_type,v.width,v.height,v.content from media_variants v join media_objects m on m.id=v.media_id join channel_memberships cm on cm.channel_id=m.channel_id and cm.user_id=? where v.media_id=? and v.variant='preview'")
                .bind(actor.to_string()).bind(media_id.to_string()).fetch_optional(&self.pool).await.map_err(sql_error)?;
            row.map(|row| {
                Ok(MediaVariant {
                    content_type: row.try_get("content_type").map_err(sql_error)?,
                    width: u32::try_from(row.try_get::<i64, _>("width").map_err(sql_error)?)
                        .map_err(storage)?,
                    height: u32::try_from(row.try_get::<i64, _>("height").map_err(sql_error)?)
                        .map_err(storage)?,
    ...
    fn activate_enrollment_invitation<'a>(
        ...
        Box::pin(async move {
            let now = Utc::now();
            let row = sqlx::query("update enrollment_invitations set state='active',authentik_invitation_id=?,activated_at=? where token_hash=? and state='inactive' and expires_at>? returning id,circle_id,invited_by,expires_at")
                .bind(command.authentik_invitation_id.to_string())
                .bind(now)
    ...
    fn create_chat_invitation<'a>(
        ...
        Box::pin(async move {
            let (target_type, circle_id, channel_id) = match &command.target {
                InvitationTarget::Circle { circle_id } => ("circle", circle_id.to_string(), None),
            ...
            let allowed: Option<i64> = if let Some(channel_id) = &channel_id {
                sqlx::query_scalar("select 1 from channels c join channel_memberships m on m.channel_id=c.id where c.id=? and c.circle_id=? and m.user_id=? and m.role in ('owner','moderator')")
                    .bind(channel_id).bind(&circle_id).bind(command.actor.to_string()).fetch_optional(&self.pool).await.map_err(sql_error)?
            } else {
                sqlx::query_scalar("select 1 from circle_memberships where circle_id=? and user_id=? and role='owner'")
                    .bind(&circle_id).bind(command.actor.to_string()).fetch_optional(&self.pool).await.map_err(sql_error)?
            };
            if allowed.is_none() {
                return Err(RepositoryError::PermissionDenied);
            }
            ...
            sqlx::query("insert into chat_invitations(id,token_hash,target_type,circle_id,channel_id,invited_by,expires_at) values(?,?,?,?,?,?,?)")
                .bind(Uuid::new_v4().to_string()).bind(hash).bind(target_type).bind(circle_id).bind(channel_id)
                .bind(command.actor.to_string()).bind(expires_at).execute(&self.pool).await.map_err(sql_error)?;
    ...
    fn inspect_chat_invitation<'a>(
        ...
        Box::pin(
            async move { load_sqlite_invitation(&self.pool, &command.actor, &command.token).await },
        )
    }
    ...
    fn decline_chat_invitation<'a>(
        ...
        Box::pin(async move {
            ...
            sqlx::query("insert into chat_invitation_responses(invitation_id,user_id,response) values(?,?,'declined') on conflict(invitation_id,user_id) do update set response='declined', responded_at=current_timestamp")
                .bind(id).bind(command.actor.to_string()).execute(&self.pool).await.map_err(sql_error)?;
            load_sqlite_invitation(&self.pool, &command.actor, &command.token).await
        })
    }
        ...
        Box::pin(async move {
            ...
            let channel_id = match &preview.target {
                InvitationTarget::Circle { circle_id } => {
                    sqlx::query("insert into circle_memberships(circle_id,user_id,role) values(?,?,'member') on conflict(circle_id,user_id) do nothing")
                        .bind(circle_id.to_string()).bind(command.actor.to_string()).execute(&mut *tx).await.map_err(sql_error)?;
                    let existing: Option<String> = sqlx::query_scalar("select id from channels where circle_id=? and lower(name)='prat' order by id limit 1")
                        .bind(circle_id.to_string()).fetch_optional(&mut *tx).await.map_err(sql_error)?;
                    let id = existing.unwrap_or_else(|| Uuid::new_v4().to_string());
                    sqlx::query("insert into channels(id,slug,name,kind,circle_id,created_by) values(?,?,'Prat','private',?,?) on conflict(slug) do nothing")
                        .bind(&id).bind(format!("{}-prat", circle_id.to_string().replace('-', ""))).bind(circle_id.to_string()).bind(command.actor.to_string())
            ...
            sqlx::query("insert into channel_memberships(channel_id,user_id,role) values(?,?,'member') on conflict(channel_id,user_id) do nothing")
                .bind(&channel_id).bind(command.actor.to_string()).execute(&mut *tx).await.map_err(sql_error)?;
            sqlx::query("insert into chat_invitation_responses(invitation_id,user_id,response) values(?,?,'accepted') on conflict(invitation_id,user_id) do update set response='accepted', responded_at=current_timestamp")
                .bind(invitation_id).bind(command.actor.to_string()).execute(&mut *tx).await.map_err(sql_error)?;
            let row = sqlx::query(
                "select id,slug,name,kind,circle_id,created_by from channels where id=?",
            )
            .bind(channel_id)
    ...
    fn delete_message<'a>(&'a self, command: DeleteMessage) -> RepositoryFuture<'a, ChatMessage> {
        Box::pin(async move {
            ...
            for statement in [
                ...
                "delete from notification_outbox where message_id=?",
            ] {
    ...
    fn list_channel_reactions<'a>(
        ...
        Box::pin(async move {
            ...
            let rows = sqlx::query("select r.message_id,r.emoji,r.user_id from message_reactions r join messages m on m.id=r.message_id where m.channel_id=? order by r.message_id,r.emoji,r.created_at,r.user_id")
                .bind(channel_id.to_string()).fetch_all(&self.pool).await.map_err(sql_error)?;
            let mut grouped = std::collections::BTreeMap::<(MessageId, String), Vec<UserId>>::new();
            for row in rows {
                let message_id = MessageId::from_uuid(
                    uuid::Uuid::parse_str(
                        &row.try_get::<String, _>("message_id").map_err(storage)?,
                    )
                    .map_err(storage)?,
                );
                let emoji = row.try_get::<String, _>("emoji").map_err(storage)?;
                let user_id = UserId::new(row.try_get::<String, _>("user_id").map_err(storage)?)
                    .map_err(storage)?;
                grouped
    ...
    fn toggle_message_reaction<'a>(
        ...
        Box::pin(async move {
            let mut tx = self.pool.begin().await.map_err(sql_error)?;
            let channel: Option<String> = sqlx::query_scalar("select m.channel_id from messages m join channel_memberships cm on cm.channel_id=m.channel_id and cm.user_id=? where m.id=? and m.deleted_at is null")
                .bind(actor.to_string()).bind(message_id.as_uuid().to_string()).fetch_optional(&mut *tx).await.map_err(sql_error)?;
            let channel_id = ChannelId::new(channel.ok_or(RepositoryError::PermissionDenied)?)
                .map_err(storage)?;
            let inserted = sqlx::query("insert into message_reactions(message_id,user_id,emoji,created_at) values(?,?,?,?) on conflict(message_id,user_id,emoji) do nothing")
                .bind(message_id.as_uuid().to_string()).bind(actor.to_string()).bind(&emoji).bind(chrono::Utc::now()).execute(&mut *tx).await.map_err(sql_error)?.rows_affected() == 1;
            if !inserted {
                sqlx::query(
...
async fn load_sqlite_invitation(
    pool: &SqlitePool,
    actor: &UserId,
    token: &str,
...
mod tests {
    ...
    async fn sqlite_repository_persists_and_reads_messages() {
        ...
        let media = repository
            .store_media(
                MediaUpload {
                    ...
                    content: b"\x89PNG\r\n\x1a\nmedia".to_vec(),
                    dimensions: None,
                    preview: Some(MediaVariant {
                        content_type: "image/jpeg".into(),
                        width: 720,
                        height: 360,
                        content: b"preview".to_vec(),
                    }),
                },
                "checksum".into(),
            )
        ...
        let preview = repository
            .load_media_preview(alice.clone(), media.id)
            .await
            .unwrap()
            .unwrap();
        assert_eq!(preview.width, 720);
        assert_eq!(preview.content, b"preview");
    ...
    #[tokio::test]
    async fn chat_invitations_are_previewable_reversible_and_scope_channel_access() {
        let repository = SqliteChatRepository::connect("sqlite::memory:")
            .await
            .unwrap();
        repository.migrate().await.unwrap();
        let owner = UserId::named("invite-owner");
        let member = UserId::named("invite-member");
        let outsider = UserId::named("invite-outsider");
        for (id, name) in [
            (owner.clone(), "Owner"),
            (member.clone(), "Member"),
            (outsider.clone(), "Outsider"),
        ] {
            repository
                .upsert_user(User {
                    id,
        ...
        let circle = repository
            .create_circle(CreateCircle {
                actor: owner.clone(),
                slug: ChannelSlug::new("invite-circle").unwrap(),
                name: DisplayName::new("Invite circle").unwrap(),
            })
            .await
            .unwrap();
        let issued = repository
            .create_chat_invitation(CreateChatInvitation {
                actor: owner.clone(),
                target: InvitationTarget::Circle {
                    circle_id: circle.id.clone(),
                },
            })
            .await
        ...
        assert!(
            repository
                ...
                .unwrap()
                .iter()
                .any(|channel| channel.id == accepted.channel.id)
        );

        let private = repository
            .create_channel(CreateChannel {
                actor: owner.clone(),
                slug: ChannelSlug::new("invite-private").unwrap(),
                name: DisplayName::new("Hemmeleg").unwrap(),
                kind: ChannelKind::Private,
                circle_id: Some(circle.id.clone()),
            })
            .await
            .unwrap();
        let channel_invite = repository
            .create_chat_invitation(CreateChatInvitation {
                actor: owner,
                target: InvitationTarget::Channel {
                    circle_id: circle.id,
        ...
        assert_eq!(
            repository
                .accept_chat_invitation(InvitationTokenCommand {
                    actor: outsider,
                    token: channel_invite.token.clone()
                })
                .await,
            Err(RepositoryError::PermissionDenied)
        );
        let joined = repository
            .accept_chat_invitation(InvitationTokenCommand {
                actor: member,
                token: channel_invite.token,
            })
            .await
            .unwrap();
        assert_eq!(joined.channel.id, private.id);
    }
}
