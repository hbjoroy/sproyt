...
impl ChatRepository for PostgresChatRepository {
    ...
    fn upsert_user<'a>(&'a self, user: User) -> RepositoryFuture<'a, User> {
        Box::pin(async move {
            ...
            let source: Option<String> =
                sqlx::query_scalar("select handle_source from users where id=$1")
            ...
            sqlx::query("insert into users (id, kind, display_name, handle, handle_source, external_provider, external_subject, created_at) values ($1, $2, $3, $4, case when $4 is null then null when $8 then 'oidc' else 'legacy' end, $5, $6, $7) on conflict(id) do update set kind = excluded.kind, external_provider = excluded.external_provider, external_subject = excluded.external_subject, handle = case when users.handle_source = 'legacy' and excluded.handle is not null and $8 then excluded.handle else users.handle end, handle_source = case when users.handle_source = 'legacy' and excluded.handle is not null and $8 then 'oidc' else users.handle_source end")
                .bind(*user.id.as_uuid())
    ...
    fn list_human_users<'a>(&'a self, actor: UserId) -> RepositoryFuture<'a, Vec<User>> {
        Box::pin(async move {
            ...
            let rows = sqlx::query("select id, kind, display_name, handle, external_provider, external_subject, created_at from users where kind = 'human' order by lower(display_name), id")
                .fetch_all(&self.pool)
                .await
                .map_err(sql_error)?;
    ...
    fn export_user_data<'a>(&'a self, actor: UserId) -> RepositoryFuture<'a, PortableUserExport> {
        Box::pin(async move {
            ...
            sqlx::query("set transaction isolation level repeatable read read only")
                ...
                .await
                .map_err(sql_error)?;
            let row = sqlx::query("select id, kind, display_name, handle, external_provider, external_subject, created_at from users where id=$1")
    ...
    fn leave_channel<'a>(&'a self, command: LeaveChannel) -> RepositoryFuture<'a, ()> {
        Box::pin(async move {
            let row: Option<(String, String, Option<uuid::Uuid>)> = sqlx::query_as(
                "select m.role,c.name,c.circle_id from channel_memberships m join channels c on c.id=m.channel_id where m.channel_id = $1 and m.user_id = $2",
            )
    ...
    fn update_channel_description<'a>(
        ...
        Box::pin(async move {
            ...
            let result = sqlx::query("update channels set description=$1 where id=$2 and exists (select 1 from channel_memberships where channel_id=$2 and user_id=$3 and role='owner')")
                .bind(&description).bind(*command.channel_id.as_uuid()).bind(*command.actor.as_uuid())
...
fn discoverable_channel_from_row(row: PgRow) -> Result<DiscoverableChannel, RepositoryError> {
    let description: String = row.try_get("description").map_err(storage)?;
...
async fn persist_mentions_postgres(
    ...
    let rows = sqlx::query(
        "select u.id,u.handle from users u join channel_memberships m on m.user_id=u.id where m.channel_id=$1 and u.kind='human'",
    )
    .bind(*message.channel_id.as_uuid())
    ...
    async fn postgres_integration_delivery_is_atomic_under_race() {
        ...
        repository
            .upsert_user(User {
                ...
                handle: Some(Handle::new("integration-race-owner").unwrap()),
        ...
        for (id, display_name) in [(alice.clone(), "Alice"), (bob.clone(), "Bob")] {
            repository
                .upsert_user(User {
                    ...
                    handle: Some(Handle::from_external(display_name)),
                    external_provider: None,
                    external_subject: None,
                    created_at: Utc::now(),
                })
    ...
    async fn presence_handoff_is_atomic_across_postgres_replicas() {
        ...
        first
            .upsert_user(User {
                ...
                handle: Some(Handle::new("presence-user").unwrap()),
                external_provider: None,
                external_subject: None,
                created_at: Utc::now(),
            })
