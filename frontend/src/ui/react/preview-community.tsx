...
export type CommunityDestination = { kind: "people" | "create-circle" | "circles" } | { kind: "channel"; channelId: string } | { kind: "channels" | "invite"; circleId: string };
export interface CommunityHost {
  ...
  createChannel(circleId: string, name: string, kind: "public" | "local" | "private"): Promise<void>;
...
function Markdown({ host, text }: { host: CommunityHost; text: string }) {
  ...
  return <LegacyContent render={render} />;
}
...
function People({ host, channel, onClose }: { host: CommunityHost; channel?: Readonly<Channel>; onClose(): void }) {
  ...
  useEffect(() => { void load(); }, []);
  const visible = people.filter(person => person.id !== host.selfId() && `${person.display_name} ${person.handle ?? ""}`.toLocaleLowerCase().includes(query.toLocaleLowerCase()));
  return <>
    ...
    {loaded && <PersonList people={visible.map(person => ({ id: person.id, name: person.display_name, detail: [person.handle ? `@${person.handle}` : "", person.status_emoji, person.status_text].filter(Boolean).join(" · ") }))}
      emptyLabel="Ingen personar passar søket." actions={person => <Button disabled={op.busy} onClick={() => void op.run(async () => { await host.openDirect(person.id); onClose(); })}>Start samtale med {person.name}</Button>} />}
...
function CircleChannels({ host, circle, channels, onClose }: { host: CommunityHost; circle: Readonly<Circle>; channels: readonly Readonly<Channel>[]; onClose(): void }) {
  ...
  const load = () => op.run(async () => setJoinable(await host.joinable(circle.id)));
  useEffect(() => { void load(); }, [circle.id]);
  return <>
    <h3>Kanalar du er med i</h3><ul>{channels.filter(channel => channel.circle_id === circle.id).map(channel => <li key={channel.id}>{channel.name}</li>)}</ul>
    <section aria-label="Ny kanal"><h3>Ny kanal</h3><form onSubmit={event => { event.preventDefault(); void op.run(async () => { await host.createChannel(circle.id, name.trim(), kind); onClose(); }); }}>
    ...
    <h3>Finn opne kanalar</h3><Button onClick={() => void load()} busy={op.busy}>Last kanalar på nytt</Button>
...
export function PreviewCommunity({ destination, snapshot, host, onClose, onNavigate }: {
  ...
  const title = destination.kind === "people" ? "Personar og ny direktemelding" : destination.kind === "channel" ? `Kanaldetaljar: ${channel?.name ?? "Kanal"}` : destination.kind === "create-circle" ? "Ny vennekrets" : destination.kind === "circles" ? "Kretsadministrasjon og invitasjonskode" : destination.kind === "channels" ? `Kanalar i ${circle?.name ?? "vennekretsen"}` : `Inviter til ${circle?.name ?? "vennekretsen"}`;
  return <Dialog open title={title} closeLabel="Lukk" onClose={onClose}>
    <div style={{ display: "grid", gap: 12 }}>
      ...
      {destination.kind === "channels" && circle && <CircleChannels host={host} circle={circle} channels={snapshot.channels} onClose={onClose} />}
