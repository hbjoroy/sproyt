...
| S-11 | Implemented, activation pending | OIDC discovery, PKCE, state, nonce, encrypted session, refresh, logout, key-rotation and revoked-user contracts in `auth.rs`; CI-tested provider verifier; live `sproyt` issuer discovery verified 2026-07-17; `authentik.md` acceptance runbook | Confirm client/redirect configuration and run the browser login, refresh, logout, revocation and key-rotation contract |
