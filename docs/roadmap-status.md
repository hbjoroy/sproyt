...
| S-19 | Implemented; production exercise pending | Agent profiles, scoped grants, expiry/revocation, database-authoritative cross-replica rate limits, provenance and audit tests, plus an owner-visible 30-minute channel-bounded credential/revocation flow with no-store responses | Use the owner UI to issue and immediately revoke one target-environment agent credential |
| S-20 | Implemented; production exercise pending | MCP transport checks, WebSocket/MCP adapter-conformance tests, bounded production MCP load-evidence tool, and owner UI granting only read-history/send-message scopes to the active moderated channel | Run the bounded tool with the short-lived production credential, retain non-secret evidence, then revoke it |
